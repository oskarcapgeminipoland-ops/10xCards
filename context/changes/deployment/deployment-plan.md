...
**Status:** Deployed to production. All phases (1–9) complete, including a live-verified rollback/roll-forward round trip in Phase 8. See the audit trail at the end of Phase 9 for the full outcome record.
...
- [N/A] **Edge case — multiple Cloudflare accounts on one login:** `wrangler whoami` lists all associated Account IDs. Make sure the `account_id` written into `wrangler.jsonc` in Phase 4 matches the intended one, not just whichever comes first/default — **did not apply**: `wrangler whoami` showed exactly one Account ID (`506febe4f03af4c1fb9b1640f0183668`), no ambiguity to resolve
- [N/A] **Edge case — headless/CI environment:** `wrangler login`'s browser flow doesn't work in CI. CI instead authenticates via the `CLOUDFLARE_API_TOKEN` environment variable (already the plan for Phase 7's GitHub Actions step) — no interactive login there, and no `wrangler login` should ever run in a CI job — **satisfied by design elsewhere**: Phase 7's `deploy` job authenticates via `CLOUDFLARE_API_TOKEN` exclusively, never calls `wrangler login`
- [N/A] **Edge case — SSO/enterprise Cloudflare accounts:** browser OAuth may route through an SSO provider and can hang or fail in some sandboxed/remote-desktop setups. If so, skip `wrangler login` entirely: create the API token directly via the dashboard (Phase 3) and confirm it works with `CLOUDFLARE_API_TOKEN=<token> npx wrangler whoami` instead — **did not apply**: plain OAuth browser flow worked on the first attempt, no SSO involved
...
- [x] `npx wrangler rollback [deployment-id]` to a prior version; confirm the site serves the older version — **done** (revisited and completed later the same session, superseding the initial skip decision below). Rolled back live from version `80f25391-cf99-4df0-af72-d1c42eaf239e` (text "Deployment test... 2") to the prior `59f6625e-0bc0-45b0-a18c-b74593d52b62` (text "Deployment test..."). This time there was a genuinely visible UI difference to verify against — the user had just pushed a deliberate one-word text change (commit `4b9dcc5`) specifically to make the drill observable, rather than rolling back between two code-identical versions as originally available. Run via `! `-prefixed command (user's own terminal, since `wrangler rollback` is blocked from direct agent execution by the Claude Code permission classifier). User confirmed via hard-refresh: text reverted to "Deployment test..." (no "2") — rollback confirmed working end-to-end, not just at the API level.
- [x] Roll forward again; confirm back on latest — **done**, rolled forward to `80f25391-cf99-4df0-af72-d1c42eaf239e`; user confirmed via hard-refresh the text was back to "Deployment test... 2". Full round trip (rollback → visible change → roll-forward → visible change back) verified live.

**Original skip decision (2026-08-21, superseded above):** initially explicitly skipped by user decision. Rationale at the time: (1) `wrangler rollback` is blocked from direct agent execution by the Claude Code permission classifier — would require manual `!`-prefixed execution regardless; (2) the two candidate versions in the account's history at that point were code-identical (same build artifact, the earlier one just failed post-upload on the missing-subdomain gate before the subdomain was registered), so the drill would have exercised the command's mechanics but shown no visible behavioral difference; (3) user's stated reasoning: this project's deploy strategy only ever *adds* new versions forward, it doesn't take the app offline, lowering the urgency of rehearsing this specific muscle memory right then. Superseded once the user pushed a small deliberate UI-visible commit specifically to make the drill worth doing properly.
...
- Phase 8's rollback drill was initially skipped, then completed later the same session once the user pushed a small deliberate UI-visible change specifically to make the drill's before/after observable — see Phase 8 above for the full round trip
...
- **Astro 6→7 major bump — explicitly deferred to a later project phase (user decision, 2026-08-21).** Not a Phase 2/4 cleanup item — it's a deliberate, scheduled-for-later upgrade, not an oversight. Closes the remaining 4 `npm audit` advisories (Astro XSS + transitive `esbuild`/`sharp`) and unlocks `@astrojs/cloudflare@14.x` (the `astro:env`-vars fix, currently moot for this project anyway since secrets are `getSecret`-routed). Do this as its own dedicated change — a major-version bump touches the whole rendering pipeline and deserves its own testing pass, not a rider on a deployment-plan phase. Concrete forcing functions to watch for: flashcard generation shipping (renders AI/user content, which is exactly what the open XSS advisories concern), or `@astrojs/cloudflare` 13.x falling out of security support.
...
- ~~CRLF/line-ending fix~~ — **done and shipped.** `.gitattributes` (`* text=auto eol=lf`) added and `npm run lint:fix` run locally post-deploy (2026-08-21) to fix a Windows checkout issue (`core.autocrlf=true` + no `.gitattributes` meant every tracked text file checked out as CRLF, which `prettier/prettier` rejects — CI never saw this since `ubuntu-latest` runners aren't affected). Committed by the user in `da393b5` ("standardize EOL, manual test of deploy") and pushed to `main`; CI ran green. `git status`/`git diff` confirmed clean afterward for every previously CRLF-flagged file, `npm run lint` clean (exit 0).
...
**Status against this bar:** fully proven, no exceptions. Phase 6's smoke test hit every one of sign-up → confirm-email → sign-in → `/dashboard` cleanly, zero error signatures in `wrangler tail`. The automated half — a `main` push triggering CI → build → unattended `wrangler deploy` — was exercised live multiple times: several commits pushed to `main`, `ci` and `deploy` jobs went green end-to-end each time, `wrangler deployments list` reflected every resulting deployment, and the live URL answered `HTTP 200` right after. The `wrangler rollback` rehearsal was also completed live, with a genuinely visible UI difference to verify against (a deliberate one-word text change, rolled back and forward, confirmed by the user via hard-refresh both times) — see Phase 8.
