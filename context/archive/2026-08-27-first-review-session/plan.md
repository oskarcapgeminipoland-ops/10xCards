...
- A module-level `scheduler = fsrs({ request_retention: 0.9, enable_short_term: false })` — `enable_short_term: false` is a required correction to this plan's original `{ request_retention: 0.9 }`-only config: the installed `ts-fsrs@5.4.1`'s `Card` type carries a `learning_steps` counter this plan's research missed, which only matters for sub-day scheduling this app doesn't use (day-granularity `intervalDays`, once-daily session cadence, no `learning_steps` column in the `flashcard_review_state` migration). Side effect worth flagging: with short-term scheduling off, `state` never actually reaches `Relearning` in practice — a lapse (`Again`) on a `Review`-state card stays in `Review` with sharply reduced `stability` instead. `Relearning` remains a valid stored value (the DB `CHECK` still allows `3`) but is not reachable through normal use under this config. (Otherwise the one fixed, non-configurable parameter set per this plan's scope.)
...
**Contract**: Covers — round-trip stability of `toCard`/`fromCard` for a reviewed card; a never-reviewed card (`row = null`) previews all 4 ratings with `dueAt` in the future; interval ordering holds for a fixed starting card (`Again` interval ≤ `Hard` ≤ `Good` ≤ `Easy`, not asserting exact FSRS constants — those are the library's responsibility); a repeated `Again` rating increases `lapses` and shrinks `stability` (**not** "moves `state` toward `Relearning`" as originally planned — `enable_short_term: false` above makes `Relearning` practically unreachable, so the test asserts the reachable proxy behavior instead; see the Phase 2 scheduler-wrapper contract for why).
...
- `getReviewSession(supabase, userId): Promise<ReviewSessionResponse>` — as implemented, runs two plain `select("*")` queries (`flashcards`, `flashcard_review_state`, both scoped to the caller by RLS alone — no app-level `.eq("user_id", ...)` filter, consistent throughout the function) and stitches "due" (state row absent or `due <= now()`) and ordering (`due ASC NULLS LAST`: overdue-reviewed cards first, never-reviewed cards fill remaining slots) together in application code, rather than a single SQL `LEFT JOIN` — see Performance Considerations for why. Capped at `DAILY_REVIEW_LIMIT`; for each row, builds a `Card` via `toCard` and attaches `previewAll(card, now)`. Also runs a cheap `select id from flashcards limit 1` (RLS-scoped, no explicit filter) to populate `hasAnyFlashcards` (distinct from whether `items` is empty). The `userId` parameter itself goes unused by this function's queries (RLS does the scoping) — kept for signature parity with `submitReview`.
- `submitReview(supabase, userId, flashcardId, rating): Promise<SubmitReviewResponse | null>` — **first** does a RLS-scoped `select id from flashcards where id = flashcardId`; zero rows means `flashcardId` doesn't exist or isn't owned by `userId` (indistinguishable, matching the `[id].ts` not-found-vs-not-owned convention), and the function returns `null` before touching `flashcard_review_state` at all — the caller (the route) maps `null` to `jsonError("Flashcard not found", 404)`, matching `updateFlashcard`/`deleteFlashcard`'s null-return convention in `flashcards.ts` rather than throwing `ApiError` from the service layer. Only then: fetches the existing `flashcard_review_state` row for `flashcardId` (if any), builds the current `Card` via `toCard`, calls `applyRating`, upserts the result (`on conflict (flashcard_id)`) with `user_id` set from the authenticated caller (never from the request body), returns the new `dueAt`/`state`. This ownership check is load-bearing, not incidental: `flashcard_review_state`'s RLS only validates the *new row's own* `user_id` on insert, never what `flashcard_id` points to — without this pre-check a caller could create a review-state row against another user's flashcard (see Critical Implementation Details).
...
The due-card query is capped at 20 rows returned to the client. As implemented, `getReviewSession` (`src/lib/services/flashcard-reviews.ts`) does **not** use the `(user_id, due)` index the Phase 1 migration added: it runs two unbounded `select("*")` queries (`flashcards`, `flashcard_review_state`) and stitches the due-filter/sort/`DAILY_REVIEW_LIMIT` cap in application code, rather than a single indexed SQL `LEFT JOIN`. This deviates from this section's original plan — see the file's own header comment for the rationale (PostgREST's embedded-resource syntax can't cleanly express "row is null OR due ≤ now" across a to-many embed, and a live-Postgres alternative wasn't verified in the implementation environment). Accepted as-is at the PRD's `target_scale.data_volume: small` — both selects are bounded by the user's own deck size, not by the review queue. The index remains defined but currently unused; revisit (e.g. via a `supabase.rpc()` function) if deck sizes grow enough for this to matter.
...
- [x] 1.5 RLS smoke test: user A's row not selectable by user B (verified via Phase 3's 3.6 cross-user check) — e767b91
...
- [x] 3.4 `GET /api/flashcards/review/session` returns expected queue, ordering, and 20-item cap — e767b91
- [x] 3.5 `POST /api/flashcards/review/submit` persists correctly for each of the 4 ratings — e767b91
- [x] 3.6 Cross-user `flashcardId` submission returns 404 — e767b91
...
- [x] 4.4 Full session walkthrough via nav entry point works end-to-end — e767b91
- [x] 4.5 Completion screen shows correct tally and working return link — e767b91
- [x] 4.6 Both empty states verified independently — e767b91
- [x] 4.7 Submit failure blocks + shows retryable error without silent advance — e767b91
