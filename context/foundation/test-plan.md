...
3. **Ryzyka to scenariusze, nie lokalizacje w kodzie.** Ten plan dokumentuje
   ...
   dokumentów, wywiadu i _sygnału_ z kodu (churn, struktura, baza testów). NIE
...
_dowód, który wyniósł to ryzyko na wierzch_ — nigdy konkretny plik jako „miejsce,
...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Użytkownik wkleja poprawny tekst i dostaje **pusty lub bezużytecznie krótki** zestaw propozycji fiszek — potok (zdjęcie code-fence → walidacja per-item → cap do 5 → `droppedCount`) po cichu odrzuca wszystko, albo zmiana promptu psuje kontrakt JSON. Ciche `[]` / all-invalid nie daje błędu; a gdy poprawny JSON przychodzi w otoczce prozy, potok zawodzi _closed z mylącym błędem_ (widoczne 502), mimo że AI odpowiedziało sensowną treścią. | High   | High       | wywiad Q1 + Q3 + Q4; `context/archive/2026-08-25-ai-flashcard-generation/plan.md` (Testing Strategy: logika parsowania = „highest-value first target"); hot-spot `src/lib/services/` (10 commitów/30d)                                                                                                               |
...
| --- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------ | ---------------------------------------------- | ----------- | ----------------------------- |
| 1   | Potok generowania AI — czysta walidacja i parsowanie           | Poprawny tekst nigdy po cichu nie daje pustego zestawu; adversarialne odpowiedzi LLM nie rzucają wyjątku                             | #1, #5 (część)           | unit                                           | complete    | testing-ai-generation-parsing |
...
- **Kolokacja**: `<module>.test.ts` obok testowanego pliku (Vitest `include`
  pokrywa `src/lib/fsrs/`, `src/lib/services/`, `src/lib/schemas/`).
- **Wzorzec „import-clean parsing module"**: jeśli logika, którą chcesz testować,
  żyje w pliku importującym `astro:env/server` (bezpośrednio lub przez
  `openrouter.ts`), wydziel czyste funkcje do osobnego modułu-rodzeństwa bez tego
  importu — wtedy test to zwykły `import` + `vitest`, bez `vi.mock` i bez
  runtime Astro. Referencja: `flashcard-generation-parse.ts` (czyste) wydzielone
  z `flashcard-generation.ts` (woła `openrouter.ts`).
- **`it.each` dla tabel właściwości / przypadków brzegowych** — jeden
  sparametryzowany test na własność zamiast sześciu prawie identycznych `it`
  (anti-pattern „redundant copies”, §1).
- **Asercje na obiekcie wyniku `{ data, error }`**, nie `expect(...).toThrow()` —
  warstwa parse/validate w tym projekcie nigdy nie rzuca (idiom Result).
- **Reference testy**: `src/lib/fsrs/scheduler.test.ts` (czysta logika FSRS),
  `src/lib/services/flashcard-generation-parse.test.ts` i
  `src/lib/schemas/flashcard.test.ts` (potok generowania + schematy).
...
- **Buduj adversarialne stringi `content` ręcznie**, każdy z **niezależnie
  znanym** oczekiwanym wynikiem `{ data: null, error: <jeden z dwóch dokładnych
stringów> }` — `"AI response was not valid JSON"` albo `"AI response was not a
JSON array"`. Warianty free-tier do pokrycia: fence + proza wiodąca, zdanie po
  zamykającym fence, uppercase ` ```JSON `, ucięcie w połowie tablicy, obiekt
  zamiast tablicy, pusty / sam whitespace.
- **Asertuj `expect(() => parseGeneratedContent(input)).not.toThrow()` obok
  kształtu wyniku.** Nigdy nie porównuj do nagranej realnej odpowiedzi LLM —
  to oracle problem (asercja pinuje bieżące zachowanie, w tym bugi).
- **Dwa zablokowane kontrakty** (dokumentują bieżące zachowanie, nie „naprawę"):
  `[]` / all-invalid → `error: null` + `proposals: []` (`droppedCount` to jedyny
  sygnał, że coś odpadło); `>5 poprawnych` → `slice` do 5 z `droppedCount: 0`
  (cap-overflow nie jest liczony — oracle: `archive/2026-08-25-ai-flashcard-generation/plan.md:115`).
- Reference: `src/lib/services/flashcard-generation-parse.test.ts`.
...
**Faza 1 — Potok generowania AI (parsowanie + schematy)** — `2026-08-29`:

- Fixtures / adversarialne stringi żyją **inline** w każdym pliku testowym
  (żadnego katalogu `__fixtures__`, żadnej zależności testowej — Vitest built-ins).
- Konwencja `it.each`: tablica wierszy jako `const NAZWA_CASES` z polem `name`,
  interpolowanym w tytule przez `$name`.
- Test parytetu limitów czyta migrację `.sql` **z dysku** (`node:fs` +
  `import.meta.url`-relatywna ścieżka `../../../supabase/migrations/…`) jako
  niezależny oracle DB. Założenie: CHECK długości pytania/odpowiedzi jest
  zdefiniowany raz w migracji create-table i nigdy nie zmieniany przez późniejszy
  `ALTER TABLE` — komentarz w teście mówi, jak rozszerzyć (glob + ostatni match),
  gdyby to przestało być prawdą.
- Rozbieżność plan↔kod: `stripCodeFence("```JSON…```")` **zdejmuje** zewnętrzny
  fence, ale zostawia słowo `JSON` przyklejone do payloadu (nie „zwraca bez
  zmian", jak zakładał plan); end-to-end i tak fail-closed. Test asertuje realny
  output.
...
- **Renderowanie UI / komponenty React / snapshoty / style** — brak testów
  ...
  `Welcome.astro`, animacje landingu. _Logikę zasilającą listę_ (matematyka
